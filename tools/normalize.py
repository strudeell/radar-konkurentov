...
вырезаем, лежит рядом в noise.yaml человеческим языком.
...
def load_noise() -> dict:
    """Словарь шумодава из noise.yaml. Компилируется один раз за прогон."""
    global _noise_cache
...
def to_snapshot(body: str, host: str = "", is_html: bool = True,
    ...
    число — цена и трогать её нельзя. Подробности в noise.yaml.
