...
слова рынка — в config/rules.yaml рядом с этим файлом. В коде остаётся только то, что
от рынка не зависит.
...
срабатывает. Почему «содержательных», а не «первых подряд» — в config/rules.yaml,
...
# Telegram и ВКонтакте. В config/sources.yaml они зовутся telegram-<id> и vk-<id>,
...
def load_rules(path: Path) -> dict:
    """Словарь из config/rules.yaml с уже скомпилированными регулярками."""
...
def judge(item: dict, rules: dict, old_lines: list[str] | None = None,
    ...
    """Классифицировать одно изменение из work/diffs/<домен>/<страница>/<дата>.json.
