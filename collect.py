...
Раз в день заходит на все страницы и каналы из config/sources.yaml, вынимает из каждой
видимый текст, очищает его от самоменяющегося и кладёт в файл:

    work/snapshots/<домен>/<страница>/<ГГГГ-ММ-ДД>.txt
...
итог по каждому — в work/runs/<ГГГГ-ММ-ДД>.json.
...
# Где что лежит. То, что задаёт человек, — в config/; то, что радар пишет
# сам, — в work/. Пути считаются от этого файла, а не от текущей папки:
# радар запускают и руками, и планировщиком, а у него папка своя.
CONFIG = ROOT / "config"
WORK = ROOT / "work"
...
# Значения по умолчанию. Человек меняет их в config/config.yaml, раздел collect.
DEFAULTS = {
...
def load_yaml(name: str) -> dict:
    path = CONFIG / name
    if not path.exists():
        sys.exit(f"Не найден {name} в папке config рядом с collect.py.")
...
def merge_into_day(report_path: Path, fresh: list[dict]) -> list[dict]:
    ...
    Зачем. Отчёт work/runs/<дата>.json — это состояние дня, а не одного запуска.
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        print(f"\nОтчёт прогона: work/runs/{today}.json")
